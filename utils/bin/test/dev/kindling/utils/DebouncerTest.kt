...
class DebouncerTest {
    ...
    fun `trailing debouncer emits last value after quiet period`() = runTest {
        val debouncer = KDebouncer<String>(this, delay = 300.milliseconds)
    ...
    fun `multiple quiet windows produce multiple emissions`() = runTest {
        val debouncer = KDebouncer<Int>(this, delay = 200.milliseconds)
...
class ThrottlerTest {
    ...
    fun `throttler emits first value immediately`() = runTest {
        val throttler = KThrottler<Int>(this, period = 500.milliseconds)
        val results = mutableListOf<Int>()
